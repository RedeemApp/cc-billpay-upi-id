...
export default function UPIIDGenerator() {
  ...
  const AMEXDetails = (): string => {
    if (cardNumber.replace(/\s+/g, "").length !== 15) {
      return "Not applicable for 16-digit cards";
    } else if (!(cardNumber.startsWith("37") || cardNumber.startsWith("34"))) {
      return "Not applicable for non-AMEX cards";
    }
    return `AEBC${cardNumber.replace(/\s+/g, "")}@SC`;
  };
      ...
      <div className="container relative sm:mt-0 max-w-4xl bg-white dark:bg-gray-800 dark:text-gray-100 p-8 rounded-lg overflow-auto">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            <img
              className="mr-2 w-8 h-8 sm:w-12 sm:h-12"
              src="upi.svg"
              alt="UPI logo"
            />
        ...
        <div className="sub-heading sm:text-xl text-purple-600 dark:text-purple-300 mb-6">
          An open-source and secure alternative to mainstream credit card
          balance payment apps.
        </div>
        ...
        <div>
          {showOverlay && (
            <div
              id="overlay"
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-[rgb(185,185,185,0.97)] p-8 rounded-lg"
            >
